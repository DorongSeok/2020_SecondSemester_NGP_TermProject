...
{
	memcpy(Val, sc_packet_user.table[(int)PlayerNum].t, sizeof(Val));

	nextBlock = static_cast<eBlock>(sc_packet_user.nextBlock[(int)PlayerNum]);
}
...
int CTable::getRandomXPos()
{
	...
	int targetPosCnt = 0;
	for (int i = 0; i < table_WIDTH; ++i) {
		if (Val[i][table_HEIGHT - standard_line] == eBlock::BLOCK_NONE
			|| Val[i][table_HEIGHT - standard_line] == eBlock::BLOCK_SHADOW) {
